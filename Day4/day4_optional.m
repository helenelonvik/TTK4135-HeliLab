...
xl(2)   = -0.6;                          % Lower bound on travel_dot
xu(2)   = 0.2;                           % Upper bound on travel_dot
xl(6)   = -0.05;                         % Lower bound on elev_dot
xu(6)   = 0.06;                          % Upper bound on elev_dot
...
figure();
f = load('pitch_10_opt.mat');
plot(f.ans(1,:), f.ans(2,:), t(1:80), 180*x3(1:80)/pi,'r--','Linewidth', 1); grid on;
legend('p^*', 'p');
currentFigure =gcf;
title(currentFigure.Children(end),'p^* vs p, with constraints');


figure();
f = load('travel_10_opt.mat');
plot(f.ans(1,:), f.ans(2,:), t(1:80), 180*x1(1:80)/pi,'r--','Linewidth', 1); grid on;
legend('{\lambda}^*', '{\lambda}');
currentFigure =gcf;
title(currentFigure.Children(end),' {\lambda}^* vs {\lambda}, with constraints');

figure();
f = load('elevation_10_opt.mat');
plot(f.ans(1,:), f.ans(2,:), t(1:80), 180*x5(1:80)/pi,'r--','Linewidth', 1); grid on;
legend('e^*', 'e');
currentFigure =gcf;
title(currentFigure.Children(end),'e^* vs e, with constraints');