 ...
 * C source code generated on : Mon Feb  8 11:14:38 2021
 *
    ...
    /* ToFile: '<S6>/To File3' */
    {
      if (!(++helicopter_DW.ToFile3_IWORK.Decimation % 1) &&
        ...
        if (fp != (NULL)) {
          ...
          if (fwrite(u, sizeof(real_T), 1 + 1, fp) != 1 + 1) {
            rtmSetErrorStatus(helicopter_M,
                              "Error writing to MAT-file pitcg_rate.mat");
            return;
          }
          ...
          if (((++helicopter_DW.ToFile3_IWORK.Count) * (1 + 1))+1 >= 100000000)
          {
            (void)fprintf(stdout,
                          ...
                          "    the simulation has ended, because it has reached\n"
                          "    the maximum number of elements (100000000)\n"
                          "    allowed in MAT-file pitcg_rate.mat.\n");
          }
        }
      }
    }
...
void helicopter_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "pitcg_rate.mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(helicopter_M, "Error creating .mat file pitcg_rate.mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp, 1 + 1, 0, "ans")) {
      rtmSetErrorStatus(helicopter_M,
                        "Error writing mat file header to file pitcg_rate.mat");
      return;
    }
...
void helicopter_terminate(void)
{
  ...
  /* Terminate for ToFile: '<S6>/To File3' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "pitcg_rate.mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(helicopter_M, "Error closing MAT-file pitcg_rate.mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(helicopter_M,
                          "Error reopening MAT-file pitcg_rate.mat");
        return;
      }
      ...
      if (rt_WriteMat4FileHeader(fp, 1 + 1, helicopter_DW.ToFile3_IWORK.Count,
           "ans")) {
        rtmSetErrorStatus(helicopter_M,
                          "Error writing header for ans to MAT-file pitcg_rate.mat");
      }
...
RT_MODEL_helicopter_T *helicopter(void)
{
  ...
  /* External mode info */
  helicopter_M->Sizes.checksums[0] = (1271575754U);
  helicopter_M->Sizes.checksums[1] = (2825488672U);
  helicopter_M->Sizes.checksums[2] = (1603072696U);
  helicopter_M->Sizes.checksums[3] = (602798694U);
